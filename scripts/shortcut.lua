...
--- panel (data.settings.mod), kept in sync both ways through State.set_player_master.
...
--- Blocked by either admin control (global "Everyone" off or a per-player
--- lock) the same way relative.lua's checkboxes are — see admin_blocked.

local State = require('__lazy-bastards-friend__.scripts.state')

local Shortcut = {}

--- Keep the toggle indicator and clickability in sync with the player's
--- master switch and any admin-side block. Registered as a State refresh
--- handler. Unavailable disables the toolbar button itself; the hotkey still
--- reaches Shortcut.toggle, which re-checks and blocks it there.
...
function Shortcut.toggle(player)
    -- Mirrors the greyed-out master switch: blocked by the global "Everyone"
    -- switch or a per-player admin lock, same as the relative panel's checkboxes.
    local data = State.get_player_data(player.index)
    local _, reason = State.tree:admin_blocked(storage.settings, data.settings, 'mod')
    if reason then
